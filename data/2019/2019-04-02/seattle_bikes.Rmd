---
title: "Bike usage in Seattle 2014 - 2019"
author: "Chris Shaw"
date: "12 April 2019"
output: html_document
abstract: "_This paper explores whether Seattle will reach its goal of quadrupling bike usage by 2030, as expressed in their Bike Master Plan.  The data collected from city bike counters forms the basis of the projection model.

Overall bike usage is decline across both commuter and leisure users, with the exception of some trails which are projected to show some modest growth in recreational use. However, the goal of quadrupling will not be achieved without some change in the patterns seen over the last five years._"
---
...
## Data collection
The raw dataset contains counts of bikes and pedestrians in Seattle at hourly time intervals over a five year period.  Counters were placed on different routes in the city and measure the direction of travel
...
bike_traffic <- bike_traffic %>%
                mutate(datetime=mdy_hms(date),
                       ...
                       hour=hour(datetime),
                       day=ordered(weekdays(datetime),levels=c("Monday", "Tuesday", "Wednesday", "Thursday", 
"Friday", "Saturday", "Sunday")),
                       ...
                       usage=if_else(hour %in% c(22,23,0,1,2,3,4,5,6), 'overnight',
                                      if_else(((hour %in% c(6,7,8,16,17,18)) & weekend==FALSE), 'commuter', 'leisure' ))
                        ) %>%
...
training <- bike_traffic %>%
  ...
  mutate(ds=week, 
         floor=0) %>%
  select (ds,usage,crossing,y, floor) %>%
  group_by(usage,crossing) %>%
  do(predict(prophet(.), make_future_dataframe(prophet(.), periods = 600, freq='week'), growth = 'logistic')) %>%
